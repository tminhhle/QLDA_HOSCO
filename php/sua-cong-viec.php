...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($stmt->execute()) {
        // Kiểm tra đồng bộ trạng thái dự án
    $sql_check = "SELECT 
    ...
    COUNT(*) AS total
  FROM congviec WHERE MaDA = ?";
$stmt_check = $conn->prepare($sql_check);
$stmt_check->bind_param("s", $maDA);
$stmt_check->execute();
...
if ($new_status) {
$sql_update_project = "UPDATE duan SET TrangThai = ? WHERE MaDA = ?";
$stmt_update_project = $conn->prepare($sql_update_project);
$stmt_update_project->bind_param("ss", $new_status, $maDA);
$stmt_update_project->execute();
}
        echo "<script>alert('Sửa công việc thành công!'); window.location.href = '../php/quan-ly-cong-viec.php?search=" . urlencode($keyword) . "';</script>";
...
<head>
    ...
    <link rel="stylesheet" href="../CSS/sua-cong-viec.css">
